...
-- 2.Give manoj the right to give other user the right to query the orders teble--
...
WHERE rating NOT IN
    (SELECT min(rating)
     FROM customers);
create or replace view restrictmin as select *from customers where rating not in (select min(rating) from customers);
